#!/usr/bin/env python3
"""Build phase-1 tuning, main, or optional phase-2 ablation manifests.

The formal phase-1 main manifest emits 144 runs: 90 LAGTCN graph runs and
54 external-baseline runs. DeepHGNN and component ablations are optional
...
PHASE2_MODEL_CONFIGS = (
    dict(
        model="DEEPHGNN_SPECTGNN", graph="H", gnn="gegenconv", temporal="tgc",
        source="project", role="end_to_end_coherent",
    ),
)
...
def formal_run_label(stage: str, item: dict, hp: dict, seed: int, graph_hp=None) -> str:
    ...
    if stage == "ae_final_tuning_v1":
        ...
        if model not in {"DLINEAR", "DEEPHGNN_SPECTGNN"}:
            parts.append(f"d{int(hp['hidden_dim'])}")
        if model == "DEEPHGNN_SPECTGNN":
            parts.append(
                "w" + _compact_decimal(hp["deephgnn_hierarchical_loss_weight"])
            )
    ...
    elif stage == "ae_phase2_ablation_v1":
        variant_aliases = {
            "no_cross_node": "noedge",
            "no_level": "nolvl",
            "no_coevolution": "noco",
            "uniform_fusion": "uniform",
        }
        variant = str(hp.get("variant") or "").lower()
        if variant:
            parts.append(variant_aliases.get(variant, variant))

    return shorten_existing_run_label("_".join(parts))


def common_command(
    args, dataset, seed, item, hp, experiment_id, tuning=False, ablation=False,
    ...
    stage = (
        ...
        else "ae_phase2_ablation_v1" if ablation
    ...
    namespace_base = namespace_override or (
        "ae/j0" if tuning
        else "ae/phase2/ablation" if ablation
        else "ae/main"
    )
    ...
    if model == "DEEPHGNN_SPECTGNN":
        cmd += [
            ...
            str(hp.get("deephgnn_hierarchical_loss_weight", 1.0)),
            "--spectgnn-alpha", str(hp.get("spectgnn_alpha", 1.2)),
            "--spectgnn-degree", str(hp.get("spectgnn_degree", 4)),
            "--spectgnn-modes", str(hp.get("spectgnn_modes", 5)),
            "--spectgnn-trend-window", str(hp.get("spectgnn_trend_window", 24)),
        ]
    if tuning:
...
def main():
    ...
    parser.add_argument(
        "--matrix", choices=("tuning", "main", "phase2-ablation"), required=True
    )
    ...
    selection_metadata = {}
    if args.matrix in {"main", "phase2-ablation"}:
    ...
    if args.matrix == "main":
        ...
        for dataset in datasets:
            for item in tuning_items:
                if item["model"] == "DEEPHGNN_SPECTGNN":
                    candidates = [
                        dict(
                            lr=lr,
                            hidden_dim=1,
                            deephgnn_hierarchical_loss_weight=weight,
                            spectgnn_alpha=1.2,
                            spectgnn_degree=4,
                            spectgnn_modes=5,
                            spectgnn_trend_window=24,
                            variant=f"lr{lr}_lambda{weight}",
                        )
                        for lr in (5e-4, 1e-3) for weight in (0.5, 1.0)
                    ]
                elif item["model"] == "DLINEAR":
        ...
        ablation_items = (
            dict(model="LAGTCN", graph="I", gnn="gcn", temporal="patch_transformer",
                 source="project", role="base", ablation="none", variant="no_cross_node"),
            dict(model="LAGTCN", graph="H", gnn="gcn", temporal="patch_transformer",
                 source="project", role="base", ablation="no_level", variant="no_level"),
            dict(model="LAGTCN", graph="H", gnn="gcn", temporal="patch_transformer",
                 source="project", role="base", ablation="no_coevolution", variant="no_coevolution"),
            dict(model="LAGTCN", graph="H+S+A+D", gnn="gcn", temporal="patch_transformer",
                 source="project", role="base", ablation="uniform_fusion", variant="uniform_fusion"),
        )
        for dataset in datasets:
            hp = selected_hp(selected, dataset, "LAGTCN")
            for seed in seeds:
                for item in ablation_items:
                    item_hp = {**hp, "variant": item["variant"]}
                    label, cmd = common_command(
                        args, dataset, seed, item, item_hp, experiment_id,
                        ablation=True,
                        graph_hp=selected_graphs[dataset],
                    )
                    runs.append({
        ...
        expected = len(datasets) * len(seeds) * len(ablation_items)
        if len(runs) != expected:
            raise AssertionError(f"Built {len(runs)} ablation runs, expected {expected}.")
        if datasets == DATASETS and seeds == SEEDS and len(runs) != 36:
            raise AssertionError(f"Full ablation matrix must contain 36 runs, got {len(runs)}.")
    ...
    print(json.dumps({
        ...
        "optional_phase2_models": [item["model"] for item in PHASE2_MODEL_CONFIGS],
        "component_ablation_phase": 2,
        **selection_metadata,
        **graph_selection_metadata,
    }, indent=2))
