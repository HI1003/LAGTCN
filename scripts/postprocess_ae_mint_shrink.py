...
for search_path in (str(CODE_DIR), str(SCRIPTS_DIR)):
...
from models_baselines_temporal import (
    DLinearBaseline,
    NHiTSBaseline,
    PatchTSTBaseline,
)
from train_eval import _align_target, _make_loader
...
def _build_model(
    ...
    *,
    ...
    elif name == "DEEPHGNN_SPECTGNN":
        model = DeepHGNNSpecTGNNBaseline(
            *common,
            sum_matrix=loader.sum_matrix,
            bottom_start_idx=loader.bottom_start_idx,
            hierarchical_loss_weight=float(
                config.get("deephgnn_hierarchical_loss_weight", 1.0)
            ),
            num_timesteps_in=int(config["num_timesteps_in"]),
            gegenbauer_alpha=float(config.get("spectgnn_alpha", 1.2)),
            polynomial_degree=int(config.get("spectgnn_degree", 4)),
            num_modes=int(config.get("spectgnn_modes", 5)),
            trend_window=int(config.get("spectgnn_trend_window", 24)),
