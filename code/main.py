...
from metrics import calculate_level_metrics, compute_coherency_violation
...
from models_baselines_temporal import DLinearBaseline, NHiTSBaseline, PatchTSTBaseline, TimesNetBaseline
from models_additional_baselines import (
    AGCRNBaseline,
    DCRNNBaseline,
    DeepHGNNSpecTGNNBaseline,
    GraphDLinearBaseline,
    GraphITransformerBaseline,
    GraphPatchTSTBaseline,
    GraphTemporalAdapterBaseline,
...
from models_reconcile import (
    BottomUpDirectGCGRU,
    BottomUpLinearGCGRU,
    BottomUpNonlinearGCGRU,
    NoReconcileGCGRU,
    TopDownDirectGCGRU,
    TopDownLinearGCGRU,
    TopDownNonlinearGCGRU,
    HybridDirectGCGRU,
    HybridLinearGCGRU,
    HybridNonlinearGCGRU,
)
from reconcile_classical import apply_all_classical
from train_eval import (
    train_model,
    evaluate_model,
    evaluate_base_forecasts,
...
def _infer_value_feature_dim(data_dir: Path, value_file: str) -> int:
    ...
    if not value_path.exists():
        raise FileNotFoundError(
            ...
            "Run scripts/build_dataset_features.py first if using a calendar/weather feature set."
        )
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Train a forecasting or reconciliation-head model.")
    parser.add_argument(
        "--batch-size",
        type=int,
        default=128,
    ...
    parser.add_argument("--num-timesteps-in", type=int, default=24, help="Input window length L_in.")
    parser.add_argument("--num-timesteps-out", type=int, default=1, help="Forecast horizon L_out.")
    ...
    parser.add_argument(
        ...
        help=(
            "Training entry: GCN-GRU-LP-(NO|BUD|BUL|BUN|TDD|TDL|TDN|HYBD|HYBL|HYBN) / "
            "DLINEAR / PATCHTST / NHITS / TIMESNET / ITRANSFORMER / "
            "GRAPH_DLINEAR / GRAPH_PATCHTST / GRAPH_ITRANSFORMER / GRAPH_ADAPTER / LAGTCN / "
            "DCRNN / STGCN / GWNET / MTGNN / DEEPHGNN_SPECTGNN / AGCRN"
        ),
    )
    parser.add_argument(
        "--gnn-type",
        type=str,
        default="gcn",
        help="Spatial backbone: none/gcn/gatv2/graphsage/transformer; gegenconv is dedicated to DEEPHGNN_SPECTGNN.",
    )
    parser.add_argument(
        "--temporal-type",
        type=str,
        ...
        help=(
            ...
            "project-specific LAGTCN encoder and tgc is dedicated to DEEPHGNN_SPECTGNN."
        ),
    )
    parser.add_argument(
        "--output-namespace",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Calendar/weather tensors are generated by scripts/build_dataset_features.py."
        ),
    )
    parser.add_argument(
        "--coherency-lambda",
    ...
    parser.add_argument(
        "--deephgnn-hierarchical-loss-weight",
        type=float,
        default=1.0,
        help="DeepHGNN aggregate hierarchy-loss weight; select using validation data only.",
    )
    parser.add_argument(
        "--spectgnn-alpha",
        type=float,
        default=1.2,
        help="SpecTGNN Gegenbauer alpha (paper Electricity default: 1.2).",
    )
    parser.add_argument(
        "--spectgnn-degree",
        type=int,
        default=4,
        help="SpecTGNN Gegenbauer polynomial degree K (paper default: 4).",
    )
    parser.add_argument(
        "--spectgnn-modes",
        type=int,
        default=5,
        help="SpecTGNN selected temporal Fourier modes S (paper short-term default: 5).",
    )
    parser.add_argument(
        "--spectgnn-trend-window",
        type=int,
        default=24,
        help="Causal moving-average window for SpecTGNN fine-grained temporal filtering.",
    )
    # ----- Reconciliation-head MLP hyperparameters (BUN / TDN / HYBN) -----
    parser.add_argument(
        "--mlp-hidden-dims",
        type=str,
        default="128,64",
        help="Comma-separated hidden sizes for BUN/TDN/HYBN reconciliation MLP (e.g. '256,128,64').",
    )
    parser.add_argument(
        "--mlp-dropout",
        type=float,
        default=0.2,
        help="Dropout probability inside the reconciliation MLP.",
    )
    parser.add_argument(
        "--mlp-layer-norm",
        type=lambda v: str(v).lower() in {"1", "true", "yes", "y"},
        default=True,
        help="Whether the reconciliation MLP uses LayerNorm after each hidden layer.",
    )
    parser.add_argument(
        "--mlp-activation",
        type=str,
        ...
        help="Activation function inside the reconciliation MLP.",
    )
    parser.add_argument(
        "--mixing-hidden-dims",
        type=str,
        default="64,32",
        help="Comma-separated hidden sizes for the per-node mixing MLP used only by HYBN.",
    )
    parser.add_argument(
        "--mixing-dropout",
        type=float,
        default=0.2,
        help="Dropout probability in the HYBN mixing MLP.",
    )
    ...
    if args.model_name == "DEEPHGNN_SPECTGNN":
        valid_gnn.add("gegenconv")
        valid_temporal.add("tgc")
    ...
    def _parse_dims(value: str, arg_name: str) -> list[int]:
        dims = [int(v) for v in str(value).split(",") if v.strip()]
        if not dims:
            raise ValueError(f"{arg_name} must contain at least one hidden dimension.")
        if any(d <= 0 for d in dims):
            raise ValueError(f"{arg_name} dimensions must be positive integers.")
        return dims

    mlp_hidden_dims = _parse_dims(args.mlp_hidden_dims, "--mlp-hidden-dims")
    mixing_hidden_dims = _parse_dims(args.mixing_hidden_dims, "--mixing-hidden-dims")
    ...
    if args.require_clean_worktree and (
        ...
        raise RuntimeError(
            "Formal run requires clean tracked files and no untracked code/scripts; "
    ...
    config = {
        ...
        # Training entry / reconciliation-head family. The actual spatial and
        # temporal modules are selected by gnn_type and temporal_type below.
        ...
        'deephgnn_hierarchical_loss_weight': float(args.deephgnn_hierarchical_loss_weight),
        'spectgnn_alpha': float(args.spectgnn_alpha),
        'spectgnn_degree': int(args.spectgnn_degree),
        'spectgnn_modes': int(args.spectgnn_modes),
        'spectgnn_trend_window': int(args.spectgnn_trend_window),
        'coherency_lambda': args.coherency_lambda,
        'st_mode': st_mode,
        # Reconciliation-head MLP hyperparameters (only used by BUN / TDN / HYBN)
        'mlp_hidden_dims': mlp_hidden_dims,
        'mlp_dropout': float(args.mlp_dropout),
        'mlp_layer_norm': bool(args.mlp_layer_norm),
        'mlp_activation': str(args.mlp_activation).lower(),
        'mixing_hidden_dims': mixing_hidden_dims,
        'mixing_dropout': float(args.mixing_dropout),
    ...
    if is_formal_ae_stage(experiment_stage):
        ...
        if experiment_stage in {"ae_final_main_v1", "ae_phase2_ablation_v1"}:
    ...
    if config['model_name'] in {"DLINEAR", "PATCHTST", "NHITS", "TIMESNET", "ITRANSFORMER"} and (
    ...
    if config['model_name'] in {"GRAPH_DLINEAR", "GRAPH_PATCHTST", "GRAPH_ITRANSFORMER"} and (
        config['temporal_type'] != "gru"
    ):
        logging.warning(
            "Model %s uses its native temporal backbone and ignores --temporal-type. "
            "Use --gnn-type to select the graph module.",
            config['model_name'],
        )
    if config['model_name'] == "GRAPH_ADAPTER" and config['temporal_type'] not in {"patchtst", "itransformer"}:
        raise ValueError("GRAPH_ADAPTER requires --temporal-type patchtst or itransformer.")
    ...
    if config['model_name'] in {"DCRNN", "STGCN", "GWNET", "MTGNN", "AGCRN"} and (
    ...
    if config["model_name"] == "DEEPHGNN_SPECTGNN":
        if config["stgnn_graph_source"] != "project":
            raise ValueError(
                "DEEPHGNN_SPECTGNN uses the supplied hierarchy graph; "
                "set --stgnn-graph-source project."
            )
        if config["gnn_type"] != "gegenconv" or config["temporal_type"] != "tgc":
            logging.warning(
                "DEEPHGNN_SPECTGNN implements its canonical GegenConv/TGC backbone and "
                "ignores generic backbone flags. Recommended args: "
                "--gnn-type gegenconv --temporal-type tgc."
            )
    if config["model_name"] in {"DCRNN", "STGCN"} and config["stgnn_graph_source"] == "native":
        logging.warning(
    ...
    elif config['model_name'] == "TIMESNET":
        model = TimesNetBaseline(
            config['node_num'],
            config['input_dim'],
            config['hidden_dim'],
            config['output_dim'],
    ...
    elif config['model_name'] == "GRAPH_DLINEAR":
        model = GraphDLinearBaseline(
            config['node_num'],
            config['input_dim'],
            config['hidden_dim'],
            config['output_dim'],
    ...
    elif config['model_name'] == "GRAPH_PATCHTST":
        model = GraphPatchTSTBaseline(
            config['node_num'],
            config['input_dim'],
            config['hidden_dim'],
            config['output_dim'],
    ...
    elif config['model_name'] == "GRAPH_ITRANSFORMER":
        model = GraphITransformerBaseline(
            config['node_num'],
            config['input_dim'],
            config['hidden_dim'],
            config['output_dim'],
    ...
    elif config['model_name'] == "GRAPH_ADAPTER":
        model = GraphTemporalAdapterBaseline(
            config['node_num'],
            config['input_dim'],
            config['hidden_dim'],
            config['output_dim'],
    ...
    elif config['model_name'] == "DEEPHGNN_SPECTGNN":
        model = DeepHGNNSpecTGNNBaseline(
            config['node_num'],
            config['input_dim'],
            config['hidden_dim'],
            config['output_dim'],
            ...
            loader.global_min,
            loader.global_max,
            sum_matrix=loader.sum_matrix,
            bottom_start_idx=loader.bottom_start_idx,
            hierarchical_loss_weight=config['deephgnn_hierarchical_loss_weight'],
            num_timesteps_in=config['num_timesteps_in'],
            gegenbauer_alpha=config['spectgnn_alpha'],
            polynomial_degree=config['spectgnn_degree'],
            num_modes=config['spectgnn_modes'],
            trend_window=config['spectgnn_trend_window'],
            stgnn_graph_source="project",
        ).to(device)
        config["deephgnn_variant"] = model.backbone_variant
        config["deephgnn_paper_doi"] = model.paper_doi
        config["deephgnn_backbone_paper_arxiv"] = model.backbone_paper_arxiv
        config["deephgnn_implementation_status"] = model.implementation_status
        config["spectgnn_blocks"] = len(model.tgc_blocks)
        config["spectgnn_frequency_indices"] = [
            {
                "coarse": block.coarse_filter.frequency_indices.detach().cpu().tolist(),
                "fine": block.fine_filter.frequency_indices.detach().cpu().tolist(),
            }
            for block in model.tgc_blocks
        ]
        config["spectgnn_trend_window_source"] = (
            "project_hourly_daily_setting_source_paper_does_not_report_w"
        )
        config["prediction_role"] = model.prediction_role
    elif config['model_name'] == "AGCRN":
        model = AGCRNBaseline(
            config['node_num'],
            config['input_dim'],
            config['hidden_dim'],
            ...
            loader.global_min,
            loader.global_max,
            adaptive_emb_dim=config['adaptive_emb_dim'],
            stgnn_graph_source=config['stgnn_graph_source'],
        ).to(device)
    elif config['model_name'] == 'GCN-GRU-LP-BUD':  # 新增：自下而上直接调和
        model = BottomUpDirectGCGRU(
            config['node_num'], config['input_dim'], config['hidden_dim'], config['output_dim'],
            config['num_layers'], loader.global_min, loader.global_max,
            loader.sum_matrix,
            gnn_type=config['gnn_type'],
            temporal_type=config['temporal_type'],
        ).to(device)
    elif config['model_name'] == 'GCN-GRU-LP-NO':  # 无调和基线
        model = NoReconcileGCGRU(
            config['node_num'], config['input_dim'], config['hidden_dim'], config['output_dim'],
            config['num_layers'], loader.global_min, loader.global_max,
            gnn_type=config['gnn_type'],
            temporal_type=config['temporal_type'],
    ...
    elif config['model_name'] == 'GCN-GRU-LP-BUL':
        model = BottomUpLinearGCGRU(
            config['node_num'], config['input_dim'], config['hidden_dim'], config['output_dim'],
            config['num_layers'], loader.global_min, loader.global_max,
            loader.sum_matrix,
            gnn_type=config['gnn_type'],
    ...
    elif config['model_name'] == 'GCN-GRU-LP-BUN':
        model = BottomUpNonlinearGCGRU(
            config['node_num'], config['input_dim'], config['hidden_dim'], config['output_dim'],
            config['num_layers'], loader.global_min, loader.global_max,
            loader.sum_matrix,
            gnn_type=config['gnn_type'],
    ...
    elif config['model_name'] == 'GCN-GRU-LP-TDD':  # 新增：自上而下直接调和
        model = TopDownDirectGCGRU(
            config['node_num'], config['input_dim'], config['hidden_dim'], config['output_dim'],
            config['num_layers'], loader.global_min, loader.global_max,
            loader.decomposition_matrix, loader.mid_to_bottom_indices,
            gnn_type=config['gnn_type'],
            temporal_type=config['temporal_type'],
        ).to(device)
    elif config['model_name'] == 'GCN-GRU-LP-TDL':
        model = TopDownLinearGCGRU(
            config['node_num'], config['input_dim'], config['hidden_dim'], config['output_dim'],
            config['num_layers'], loader.global_min, loader.global_max,
            loader.decomposition_matrix, loader.mid_to_bottom_indices,
            gnn_type=config['gnn_type'],
    ...
    elif config['model_name'] == 'GCN-GRU-LP-TDN':
        model = TopDownNonlinearGCGRU(
            config['node_num'], config['input_dim'], config['hidden_dim'], config['output_dim'],
            config['num_layers'], loader.global_min, loader.global_max,
            loader.decomposition_matrix, loader.mid_to_bottom_indices,
            gnn_type=config['gnn_type'],
    ...
    elif config['model_name'] == 'GCN-GRU-LP-HYBD':  # 新增：混合直接调和
        model = HybridDirectGCGRU(
            config['node_num'], config['input_dim'], config['hidden_dim'], config['output_dim'],
            config['num_layers'], loader.global_min, loader.global_max,
            loader.sum_matrix, loader.decomposition_matrix, loader.mid_to_bottom_indices,
            config['alpha'], config['alpha_trainable'],
            gnn_type=config['gnn_type'],
            temporal_type=config['temporal_type'],
        ).to(device)
    elif config['model_name'] == 'GCN-GRU-LP-HYBL':
        model = HybridLinearGCGRU(
            config['node_num'], config['input_dim'], config['hidden_dim'], config['output_dim'],
            config['num_layers'], loader.global_min, loader.global_max,
            loader.sum_matrix, loader.decomposition_matrix, loader.mid_to_bottom_indices,
            config['alpha'], config['alpha_trainable'],
    ...
    elif config['model_name'] == 'GCN-GRU-LP-HYBN':
        model = HybridNonlinearGCGRU(
            config['node_num'], config['input_dim'], config['hidden_dim'], config['output_dim'],
            config['num_layers'], loader.global_min, loader.global_max,
            loader.sum_matrix, loader.decomposition_matrix, loader.mid_to_bottom_indices,
            gnn_type=config['gnn_type'],
            ...
            mlp_layer_norm=config['mlp_layer_norm'],
            mlp_activation=config['mlp_activation'],
            mixing_hidden_dims=config['mixing_hidden_dims'],
            mixing_dropout=config['mixing_dropout'],
        ).to(device)
    else:
    ...
    if config['model_name'].startswith('GCN-GRU-LP-'):
        base_predictions = evaluate_base_forecasts(model, test_dataset, static_edge_index, device)
    ...
    # === Classical (post-hoc) reconciliation for comparison ===
    if config['model_name'] == 'GCN-GRU-LP-NO':
        logging.info("Running classical post-hoc reconciliation methods...")
        # Compute in-sample residuals for MinT-WLS/SHR
        train_loader_for_residuals = __import__('train_eval', fromlist=['_make_loader'])._make_loader(
            train_dataset, batch_size=64, shuffle=False
        )
        train_preds_list, train_trues_list = [], []
        model.eval()
        with torch.no_grad():
            for x_b, y_b in train_loader_for_residuals:
                x_b, y_b = x_b.to(device), y_b.to(device)
                y_p = model(x_b, static_edge_index)
                y_t = model.transform_target(y_b)
                if y_t.dim() == 2:
                    y_t = y_t.unsqueeze(-1)
                if tuple(y_t.shape) != tuple(y_p.shape):
                    raise ValueError(
                        f"Classical-residual target/prediction mismatch: "
                        f"{tuple(y_t.shape)} vs {tuple(y_p.shape)}"
                    )
                train_preds_list.append(y_p.cpu().numpy())
                train_trues_list.append(y_t.cpu().numpy())
        train_preds_all = np.concatenate(train_preds_list, axis=0)
        train_trues_all = np.concatenate(train_trues_list, axis=0)
        train_residuals = train_trues_all - train_preds_all

        classical_results = apply_all_classical(
            base_forecasts=predictions,
            sum_matrix=loader.sum_matrix,
            bottom_start_idx=loader.bottom_start_idx,
            train_actuals=train_trues_all,
            residuals=train_residuals,
            mid_to_bottom_indices=loader.mid_to_bottom_indices,
        )
        from train_eval import _compute_metrics
        classical_metrics_all = {}
        for method_name, reconciled_preds in classical_results.items():
            c_metrics = _compute_metrics(
                true_values, reconciled_preds,
                num_timesteps_in=config['num_timesteps_in'],
                mase_scale=config.get("_mase_scale"),
                require_mase_scale=is_formal_ae_stage(
                    config.get("experiment_stage")
                ),
            )
            c_coherency = compute_coherency_violation(
                reconciled_preds, loader.sum_matrix, bottom_start_idx=loader.bottom_start_idx,
            )
            c_metrics.update(c_coherency)
            classical_metrics_all[method_name] = c_metrics
            logging.info("Classical %s — MAE: %.4f, RMSE: %.4f, Coherency MAE: %.6f",
                         method_name, c_metrics["MAE"], c_metrics["RMSE"], c_metrics["coherency_mae"])

        classical_path = os.path.join(config['output_dir'], artifact_filename("classical_reconcile_metrics"))
        with open(classical_path, 'w') as f:
            json.dump({k: {mk: float(mv) if isinstance(mv, (float, int, np.floating)) else mv
                           for mk, mv in v.items()} for k, v in classical_metrics_all.items()}, f, indent=4, allow_nan=False)
        logging.info("Classical reconciliation metrics saved to %s", classical_path)
    ...
    # Save the canonical output plus an explicit role-specific forecast file.
    save_predictions(predictions, true_values, config)
    if base_predictions is not None:
        save_predictions(base_predictions, true_values, config, filename=BASE_PRED_FILENAME)
        if config['model_name'] != 'GCN-GRU-LP-NO':
            save_predictions(predictions, true_values, config, filename=REC_PRED_FILENAME)
    elif config.get("prediction_role") == "end_to_end_coherent":
        save_predictions(predictions, true_values, config, filename=REC_PRED_FILENAME)
    else:
