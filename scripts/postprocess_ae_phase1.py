...
Recomputes, from saved base-forecast artifacts and WITHOUT retraining:
  ...
  B   final-definition sMASE-24 (training-period per-node 24-hour seasonal-
      naive scale, macro average per level) via code/mase.py, with the frozen
      denominators persisted per dataset.
  C   nonnegative BU / TD-FP / MinT-NNLS reconciliation with full solver
      diagnostics via code/reconcile_ae.py. Metric-only revisions may
  ...
  F2  retroactive clamp-trigger rates against the original-scale bounds
      [0, expm1(mu + 6*sigma)] baked into LAGTCN / TimesNet / Graph Adapter.
...
Usage:
    python scripts/postprocess_ae_phase1.py --runs-root <dir> [--data-root Data]
...
# Models whose forward pass clamps normalized forecasts to [0, mu+6*sigma] in
# log space (models_additional_baselines / models_baselines_temporal).
CLAMP_INTRODUCTIONS = {
    "TIMESNET": {
        "introduced_at": "2026-06-22T17:37:14",
        "commit": "b5fdae4f62c6ff7676738db00dac6e75e222929e",
    },
    "LAGTCN": {
...
def load_dataset_meta(dataset_dir: Path) -> DatasetMeta:
    ...
    if "middle_levels" not in info:
        raise RuntimeError(
            ...
            "run scripts/add_middle_levels.py first."
        )
...
def clamp_provenance(model_name: str, run_timestamp) -> dict:
    ...
    provenance. It still prevents pre-introduction TimesNet runs from being
